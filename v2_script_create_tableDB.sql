...
-- -----------------------------------------------------
-- Table `SmartStock`.`images`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `SmartStock`.`images` ;

CREATE TABLE IF NOT EXISTS `SmartStock`.`images` (
  `imageCode` INT NOT NULL AUTO_INCREMENT,
  `image` VARCHAR(100) NOT NULL,
  `fkProdCode` INT NOT NULL,
  PRIMARY KEY (`imageCode`),
  INDEX `fk_images_products1_idx` (`fkProdCode` ASC) VISIBLE,
  CONSTRAINT `fk_images_products1`
    FOREIGN KEY (`fkProdCode`)
    REFERENCES `SmartStock`.`products` (`prodCode`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
